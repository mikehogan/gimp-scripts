...
;;;    https://cartographersguild.com/showthread.php?t=533
;;;
;;;    Copyright (C) 2010-2021  Mike Hogan <themikehogan@gmail.com>
;;;
...
;;
;; Copy this script to your ~/.gimp-2.10/scripts directory as old-paper.scm
;;      Windows use "%appdata%\GIMP\2.10\scripts"
;;      or check your scripts folder in Edit->Preferences->Folders->Scripts
...
;   2021-04-15
;       updated to Gimp 2.10
;       fixed a missing (gimp-selection-all theImage) that was causeing offset error (thanks to Volkmar for reporting it)
...
(let*
    ...
    (gimp-selection-all theImage)
    (plug-in-plasma RUN-NONINTERACTIVE theImage theTextureLayer (abs (srand (realtime))) 2.5)
    (gimp-desaturate theTextureLayer)
    (plug-in-bump-map RUN-NONINTERACTIVE theImage theTextureLayer theTextureLayer 135 45 5 0 0 0 0 TRUE FALSE 0)
    ...
    (plug-in-plasma RUN-NONINTERACTIVE theImage theFuzzLayer (abs (srand (realtime))) 2.5)
